...
static bool cache_node_cache_create_cb(lv_freetype_cache_node_t * node, void * user_data)
{
    ...
    /* ===== 添加：强制设置 Unicode charmap ===== */
    FT_Error charmap_err = FT_Select_Charmap(face, FT_ENCODING_UNICODE);
    if(charmap_err) {
        LV_LOG_WARN("FT_Select_Charmap(UNICODE) failed: %d, trying all charmaps", (int)charmap_err);
        FT_Int unicode_idx = -1;
        ...
        if(unicode_idx >= 0) {
            error = FT_Set_Charmap(face, face->charmaps[unicode_idx]);
            if(error) {
                FT_ERROR_MSG("FT_Set_Charmap", error);
                FT_Done_Face(face);
                return false;
            }
        } else {
            LV_LOG_ERROR("No Unicode charmap found in font!");
            FT_Done_Face(face);
            return false;
        }
    }
    LV_LOG_INFO("Unicode charmap active: platform=%d encoding=%d", 
                face->charmap->platform_id, face->charmap->encoding);
    /* ========================================== */
